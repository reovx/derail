        ...
        <ul className="flex flex-col divide-y divide-border-soft">
          {events.map((event) => (
            <li key={event.id} className="flex flex-wrap items-center gap-3 py-2.5 first:pt-0">
              ...
              <span className="flex-1 text-body text-secondary">{describeGateEvent(event)}</span>

              <span className="text-small text-muted" title={event.at}>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="font-mono text-small text-muted underline underline-offset-2 transition-colors hover:text-foreground"
                title={event.txHash}
              >
